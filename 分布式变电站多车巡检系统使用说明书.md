...
2. **碰撞避免**：统一调度确保同一时刻只有一车尝试移动到特定格子（SETNX + TTL 5s 覆盖整个节拍）
...
4. 移动流程：peek 路径 → 检查障碍 → SETNX 目标格抢占 → pop 路径 → 更新位置 → 点亮 3×3（含视线阻挡检测）→ 递增步数 → 状态变迁
5. 路径走完、受阻、碰撞时重置状态
...
| 单步移动耗时 | 5~15ms | peek→检查→SETNX→pop→更新→点亮（含 Redis I/O） |
...
- Redis String `car_reserve:{x}:{y}`：SETNX 目标格抢占锁（TTL 5s）
...
| 抢占锁 | Redis Protocol | `car_reserve:{x}:{y}` String | SETNX |
...
| 机制 | 实现 | 用途 |
|------|------|------|
| SETNX 原子抢占 | `SETNX car_reserve:{x}:{y} <carId>` + EXPIRE 5s | 小车移动目标格抢占 |
...
- 写冲突场景：`car_reserve` SETNX 天然避免；Controller 与 Car 对 `Cars:{id}` state 字段写操作在时序上不重叠（Controller 先写后发 tick，Car 收到 tick 后才写）
...
| `car_reserve:{x}:{y}` | String | 任意值 | 移动目标格抢占锁（TTL 5s） | Car（SETNX） | Car（EXISTS 检查） |
...
| **原子操作** | `car_reserve:{x}:{y}` 使用 SETNX 保证目标格抢占的原子性 |
...
```
阶段 1: SETNX car_reserve:{targetX}:{targetY} <carId> EX 5
...
**设计亮点**：两级防护互补——SETNX 防止同 tick 内两车竞争同一格子（超时自动释放），占用检查防止竞态窗口（SETNX 成功后被其他车先到）。
