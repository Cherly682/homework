...
双击根目录的 **`start.bat`**，根据菜单选择操作。或者直接运行 PowerShell 脚本：
...
# 启动前端（另一个终端）
cd View/CREAZYTHURSDAY/com.Manny
java -jar target/com.Manny-1.0-SNAPSHOT.jar
```
...
- **应用程序日志目录**：运行时通过 `-Dlog.dir=<dir>` 指定，默认 `logs/`
  - `backend.log`：后端全链路日志（Controller 节拍、Car 每步耗时、Navigator 路径规划、Recorder 录制）
  - `frontend.log`：前端渲染和用户操作日志（登录、地图渲染、按钮点击）
- **脚本日志目录**：`scripts/logs/`，每个脚本独立记录启动/构建日志，用于调试启动问题
  - `build-all.log` / `build-backend.log` / `build-frontend.log`：构建日志
  - `run-all.log`：一键启动日志
...
-   **修好了前端启动时 JAR 签名验证报错**：maven-shade-plugin 打包时排除了依赖 JAR 的签名文件（META-INF/*.SF, *.DSA, *.RSA），避免高版本 JDK 的 JarVerifier 校验失败。
...
- **支持多主机分布式部署**：启动前自动询问 Redis/RabbitMQ 服务器地址，配置保存到 `connection.conf` 供下次复用。各主机只需指向同一服务器即可协同工作，无需修改 Java 代码。
- **新增交互式菜单多选功能**：支持逗号分隔同时启动多个组件（如 `3,A` 启动前端 + Car）。
