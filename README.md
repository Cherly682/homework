...
├── docs/
│   └── uml/
│       └── 全部UML图.md                # 全套系统架构图（119张Mermaid图）
```
...
- 碰撞避免原子化——将 SETNX+EXPIRE 两步改为 `SET key value NX EX 5` 一步原子操作，消除竞态窗口。
- 后端批量读取优化——SCAN count 从 20 提升至 500（减少迭代次数）、getCarPoint 两次 hget 合并为一次 hmget（减少 Redis 往返）、移除 setCarState 调试日志中的多余回读。
- 前端 Redis 连接自动恢复——View 全部 Model/View 类新增 `ensureConnection()` 方法，每次 Redis 操作前自动探测连接状态，断连时从连接池获取新连接，防止长时间空闲后前端卡死。
