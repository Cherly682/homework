 ...
 *
 * 【这个类是干什么的？】
 * 想象一下，你的程序需要连接 Redis 数据库、RabbitMQ 消息队列，
 * 这些服务的地址、端口、密码等配置信息不能写死在代码里——
 * 因为在不同的电脑上运行时，这些配置可能会变化。
 *
 * ConfigManager 就是"配置中心"，统一管理所有配置信息。
 * 它支持四种配置来源，按优先级从高到低分别是：
 *
 *   1. 环境变量（最高优先级）
 *      → 例如在命令行中设置：set REDIS_HOST=192.168.1.100
 *      → 适合部署到不同服务器时，动态修改配置
 *
 *   2. JVM 系统属性（-D 参数）
 *      → 例如启动时加参数：java -Dredis.host=192.168.1.100 -jar app.jar
 *
 *   3. config.properties 文件
 *      → 放在 classpath 下的配置文件，存放默认值
 *
 *   4. 代码中指定的默认值（最低优先级）
 *      → 前面三种都没找到时，使用兜底值
 *
 * 【为什么要这样设计？】
 * 这叫做"约定优于配置" + "外部化配置"的设计模式：
 * - 开发时：用 config.properties 里的默认值就够用
 * - 部署时：通过环境变量覆盖，不需要改代码
 * - 紧急情况：通过 JVM 参数临时覆盖某个配置
 *
 * 【类比理解】
 * 就像手机的"设置"App：
 * - config.properties = 出厂默认设置
 * - 环境变量     = 你手动改了 WiFi 密码（覆盖出厂设置）
 * - JVM 参数     = 用快捷开关临时开了飞行模式
 */
