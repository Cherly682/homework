...
确保 Redis 和 RabbitMQ 已运行，然后双击根目录 **`start.bat`**，或在终端执行：

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File start.ps1
```

脚本自动完成：构建后端 fat JAR → 编译前端 → 初始化 Redis 数据 → 启动后端全部模块（4 个独立进程 + Launcher）→ 启动前端 Swing 界面。日志输出到 `logs/backend.log` 和 `logs/frontend.log`。
...
- 一键启动脚本 `start.ps1` ——双击自动构建、初始化、启动后端+前端。
